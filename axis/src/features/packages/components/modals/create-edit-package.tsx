...
export const createPackageFn = createServerFn({
	...
	.handler(async ({ data }) => {
		...
		const submissionData = prepareCreatePackageData(data, session.cafeId);
		const pkg = await createPackageUseCase(submissionData);

		return pkg;
	});

interface CreateEditPackageDialogProps {
...
export function CreateEditPackageDialog({
	packageData,
	isOpen,
	onClose,
	onSuccess,
}: CreateEditPackageDialogProps) {
    const queryClient = useQueryClient();
		const isEditing = !!packageData;
		const btnLabel = isEditing ? "تحديث الباكيج" : "اضافة باكيج جديد";

		const form = useForm<PackageFormValues>({
			resolver: zodResolver(packageFormSchema),
			defaultValues: {
				name: isEditing && packageData?.name ? packageData.name : "",
				items:
					isEditing && packageData?.items
						? packageData.items.map((item) => ({
								name: item.name || "",
								price: Number(item.price) || 0,
							}))
						: [],
				isActive: isEditing ? packageData?.status === "active" : false,
			},
		});
