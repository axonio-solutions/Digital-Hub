...
export function TaxonomyHub() {
  ...
  const categoryStatuses = useMemo(
    () => [
      {
        label: t('table.columns.status_types.active', 'Active'),
        value: 'active',
        icon: () => (
          <span className="size-2 rounded-full bg-emerald-500 me-2" />
        ),
      },
      {
        label: t('table.columns.status_types.draft', 'Draft'),
        value: 'draft',
        icon: () => <span className="size-2 rounded-full bg-slate-400 me-2" />,
      },
      {
        label: t('table.columns.status_types.archived', 'Archived'),
        value: 'archived',
        icon: () => <span className="size-2 rounded-full bg-amber-500 me-2" />,
      },
    ],
    [t],
  )
  ...
  const categoryCols: Array<ColumnDef<any>> = useMemo(
    () => [
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-[10px] text-muted-foreground truncate max-w-[200px] text-start">
                {tCategoryDescription(row.original, t) ||
                  row.original.description ||
                  t('table.columns.no_description')}
              </span>
            </div>
          </div>
        ),
      },
      {
        ...
        header: t('table.columns.demand_logic'),
        cell: ({ row }) => (
          <span className="font-bold text-sm tabular-nums">
            {(row.original.requestsCount || 0).toLocaleString()}
          </span>
        ),
      },
      {
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40 rounded-xl">
                <DropdownMenuLabel className="text-[10px] font-bold uppercase tracking-wider text-slate-400 px-2 py-1.5">
                  {t('table.actions.category_label')}
                </DropdownMenuLabel>
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onClick={() => {
                ...
                <DropdownMenuItem
                  ...
                  className="rounded-lg font-bold text-xs gap-2 py-2 cursor-pointer text-rose-600 focus:bg-rose-600 focus:text-white"
                >
  ...
  const brandCols: Array<ColumnDef<any>> = useMemo(
    () => [
      ...
      },
      {
        ...
        header: t('table.columns.cluster_origin'),
        cell: ({ row }) => (
          <span className="text-sm font-medium text-foreground">
            {row.original.clusterOrigin || '-'}
          </span>
        ),
      },
      {
        accessorKey: 'clusterRegion',
        header: t('table.columns.cluster_region'),
        cell: ({ row }) => (
          <Badge variant="outline" className="text-[10px] rounded-full">
            {row.original.clusterRegion || '-'}
          </Badge>
        ),
      },
      {
        accessorKey: 'requestsCount',
        header: t('table.columns.active_demand'),
        cell: ({ row }) => (
          <span className="font-bold text-sm tabular-nums">
            {(row.original.requestsCount || 0).toLocaleString()}
          </span>
          ...
          >
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-40 rounded-xl">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem
                  onClick={() => deleteBrand.mutate(row.original.id)}
                  className="rounded-lg font-bold text-xs gap-2 py-2 cursor-pointer text-rose-600 focus:bg-rose-600 focus:text-white"
                >
                  <Trash2 className="size-3.5" />{' '}
  ...
  const handleAdd = () => {
    setEditing(null)
    activeTab === 'categories' ? setIsCategoryOpen(true) : setIsBrandOpen(true)
  }
    ...
    <div className="flex-1 flex flex-col gap-6 w-full pb-8 pt-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button
            variant="outline"
            className="h-10 px-4 rounded-xl border-slate-200 dark:border-slate-800 gap-2 font-bold text-xs uppercase tracking-wider shadow-sm"
          >
            <Download size={14} /> {t('buttons.export')}
          </Button>
          <Button
            onClick={handleAdd}
            className="h-10 px-4 rounded-xl bg-primary text-primary-foreground gap-2 font-bold text-xs uppercase tracking-wider shadow-lg shadow-primary/20"
          >
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        ...
        >
          <TabsList className="bg-slate-100/50 dark:bg-slate-900/50 p-1 border border-slate-200 dark:border-slate-700 h-10 rounded-xl">
            <TabsTrigger
              value="categories"
              className="px-4 text-[10px] font-bold uppercase tracking-wider data-[state=active]:bg-white dark:data-[state=active]:bg-slate-950 rounded-lg"
            >
