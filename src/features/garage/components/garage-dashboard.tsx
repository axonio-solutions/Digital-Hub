...
import { useVehicles, useAddVehicle, useRemoveVehicle } from "../hooks/use-vehicles"
...
export function GarageDashboard() {
  ...
  if (isLoading) return (
    <div className="flex flex-col h-64 items-center justify-center space-y-4 animate-pulse">
      ...
      <p className="text-muted-foreground font-medium">Accessing your garage...</p>
    </div>
  )
  ...
  if (isError) return (
    <div className="bg-red-50 border border-red-200 rounded-xl p-8 flex flex-col items-center justify-center text-center">
      ...
      <p className="text-red-700 max-w-sm mt-2">We couldn't reach the garage at the moment. Please check your connection.</p>
    </div>
  )
  ...
  return (
    <div className="flex flex-col space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">My Garage</h2>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogTrigger asChild>
            <Button size="lg" className="rounded-xl shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all font-semibold gap-2">
          ...
          <DialogContent className="sm:max-w-[500px] rounded-2xl">
            <DialogHeader>
              <DialogTitle className="text-2xl font-bold">Register Vehicle</DialogTitle>
              <DialogDescription>
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {userVehicles.length === 0 ? (
          <Card className="col-span-full border-dashed border-2 bg-slate-50/50 py-16 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors">
            ...
            <Button variant="outline" onClick={() => setIsDialogOpen(true)} className="rounded-xl border-2 hover:bg-slate-100">
              Click here to start
            </Button>
          </Card>
        ) : userVehicles.map((vehicle: any) => (
          <Card key={vehicle.id} className="group relative overflow-hidden border-2 hover:border-primary/30 hover:shadow-xl transition-all duration-300 rounded-3xl">
            ...
            <CardHeader className="pb-3">
              <div className="flex items-start justify-between">
                ...
                <Badge variant="secondary" className="bg-slate-100 text-slate-700 font-bold px-3 py-1 rounded-full">
                  {vehicle.year}
                </Badge>
              </div>
            ...
            <CardContent className="space-y-4 pt-2">
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">Plate</div>
                  <div className="font-mono text-sm text-slate-700 bg-white px-2 py-0.5 rounded border inline-block select-all">{vehicle.licensePlate || 'N/A'}</div>
                </div>
                <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100">
                  <div className="text-[10px] uppercase font-bold text-slate-400 mb-1">VIN</div>
                  <div className="font-mono text-xs text-slate-700 bg-white px-2 py-0.5 rounded border block truncate select-all" title={vehicle.vin}>{vehicle.vin || 'N/A'}</div>
                </div>
              </div>
            ...
            <CardFooter className="pt-2 flex gap-2">
              <Button
                ...
                onClick={() => toast.info("Vehicle details editing coming soon!")}
              >
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="bg-slate-900 text-white rounded-[2rem] p-8 relative overflow-hidden group">
          ...
          <div className="relative z-10 flex flex-col h-full justify-between">
            <div>
              ...
              <p className="text-slate-400 leading-relaxed mb-6">Soon you'll be able to track oil changes and inspection dates directly from your garage.</p>
            </div>
            <Button className="w-fit bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl px-6">Notify Me</Button>
          </div>
        </div>
        <div className="bg-white border-2 border-slate-100 rounded-[2rem] p-8 flex items-center gap-6">
          ...
          <div>
            <h3 className="text-xl font-bold text-slate-900 mb-1">Service History</h3>
            <p className="text-slate-500">View all parts requested and quotes accepted for each of your vehicles in one place.</p>
            <Button variant="link" className="p-0 h-auto text-primary font-bold mt-2 hover:no-underline">Go to History →</Button>
          </div>
        </div>
      </div>
    </div>
