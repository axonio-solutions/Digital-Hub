...
interface SellerMarketplaceGridViewProps {
    data: any[]
    onAction: (item: any) => void
    type: 'opportunity' | 'active'
}

export function SellerMarketplaceGridView({ data, onAction, type }: SellerMarketplaceGridViewProps) {
    if (data.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 bg-muted/20 rounded-2xl border-2 border-dashed border-muted">
                ...
                <p className="text-muted-foreground font-medium">No results matching your filters.</p>
            </div>
        )
    }
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {data.map((item) => {
                ...
                return (
                    <Card key={item.id} className="group overflow-hidden border-muted-foreground/10 hover:border-primary/30 hover:shadow-xl transition-all duration-300">
                        ...
                        <CardHeader className="p-4 pb-0">
                            <div className="space-y-1">
                                ...
                                <div className="flex items-center gap-2 text-muted-foreground">
                                    ...
                                    <p className="text-xs font-medium">{item.vehicleBrand} {item.vehicleModel || ""}</p>
                                </div>
                            </div>
                        </CardHeader>
