  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[92vh] p-0 border-none shadow-2xl bg-background overflow-hidden rounded-2xl">
        ...
        <div
          className={cn('h-1.5 w-full bg-gradient-to-r shrink-0', roleColor)}
        />
        <div className="flex items-center gap-4 px-5 sm:px-6 pt-4 pb-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex flex-wrap gap-1.5 mt-2">
              <AccountBadge
                label={t('users.profile.labels.role')}
                value={t(`users.roles.${role}`)}
                variant={
                  role === 'admin'
                    ? 'info'
                    : role === 'seller'
              ...
              />
              <AccountBadge
                label={t('users.profile.labels.verified')}
                value={
                  u.emailVerified
                    ? t('users.profile.pills.verified')
                    : t('users.profile.pills.pending')
                }
                variant={u.emailVerified ? 'success' : 'warning'}
              />
              <AccountBadge
                label={t('users.profile.labels.age')}
                value={`${accountAgeDays}d`}
              />
              {u.storeName && (
                <AccountBadge
                  label={t('users.profile.labels.store')}
                  value={u.storeName}
                  icon={Store}
                />
              )}
              {u.commercialRegister && (
                <AccountBadge
                  label={t('users.profile.labels.reg_number')}
                  value={u.commercialRegister}
                  icon={FileText}
                />
              )}
              {isBanned && u.banReason && (
                <AccountBadge
                  label={t('users.profile.labels.ban')}
                  value={u.banReason}
                  icon={Ban}
                  variant="error"
                />
              )}
              {isBanned && u.banExpires && (
                <AccountBadge
                  label={t('users.profile.labels.expires')}
                  value={new Date(u.banExpires).toLocaleDateString()}
                />
              )}
            </div>
          </div>
            ...
            <>
              ...
              {role === 'buyer' && (
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  <Pill
                    ...
                    label={t('users.profile.pills.requests')}
                  />
                  <Pill
                    value={accountAgeDays}
                    label={t('users.profile.pills.days')}
                  />
                  <Pill
                    value={
                      u.emailVerified
                    ...
                    label={t('users.profile.labels.email')}
                  />
                </div>
              )}
              {role === 'seller' && (
                <div className="space-y-3">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    <Pill
                      value={isLoading ? '...' : quoteCount}
                      label={t('users.profile.pills.quotes')}
                    />
                    <Pill
                      value={brandCount}
                      label={t('users.profile.pills.brands')}
                    />
                    <Pill
                      value={categoryCount}
                      label={t('users.profile.pills.categories')}
                    />
                    <Pill
                      value={priorityScore ?? '—'}
                      label={t('users.profile.pills.priority')}
                    />
                  </div>
                  {categoryCount > 0 && (
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1 mt-1">
                        {u.sellerCategories?.map((sc: any, i: number) => (
                          <Badge
                            key={i}
                            variant="secondary"
                            className="text-[10px] rounded-full px-2.5 gap-1"
                          >
                            <CategoryDisplay
              ...
              {role === 'admin' && (
                <div className="grid grid-cols-2 gap-2">
                  <Pill
                    value={accountAgeDays}
                    label={t('users.profile.pills.days_active')}
                  />
                  <Pill
                    value={
                      u.emailVerified
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-1.5">
                  ...
                  <InfoRow
                    icon={Phone}
                    label={t('users.profile.labels.phone')}
                    value={u.phoneNumber || '—'}
                    copyable={!!u.phoneNumber}
                    onCopy={() => copy(u.phoneNumber, 'phone')}
                    copied={copied === 'phone'}
                  />
                  <InfoRow
                    icon={MapPin}
                    label={t('users.profile.labels.location')}
                    value={[u.wilaya, u.city].filter(Boolean).join(', ') || '—'}
                  />
                  <InfoRow
                    icon={Calendar}
                    label={t('users.profile.labels.joined')}
        ...
        <div className="px-3 sm:px-6 py-3 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/20 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2">
          ...
          <div className="flex flex-col sm:flex-row gap-1.5">
            {role === 'seller' && (
              ...
              >
                <Coins size={14} className="me-1.5" />{' '}
                {t('users.profile.credits')}
              </Button>
            )}
            {isWaitlisted && (
              <Button
                onClick={() => {
                  activateSeller({ userId: u.id })
                  toast.success('users.profile.success.activated')
                }}
              ...
              >
                <UserCheck size={14} className="me-1.5" />{' '}
            ...
            <Button
              ...
              onClick={() => {
                toggleBan({ userId: u.id, isBanned: !isBanned })
                toast.success(
                  isBanned
                    ? 'users.profile.success.restored'
                    : 'users.profile.success.suspended',
                )
              }}
            ...
            >
              {isBanned ? (
                ...
                <>
                  <UserX size={14} className="me-1.5" />{' '}
...
function Pill({ value, label }: { value: string | number; label: string }) {
  return (
    <div className="flex flex-col items-center justify-center gap-0.5 py-2 rounded-xl bg-slate-50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800/50">
      <span className="text-sm font-black tabular-nums leading-none text-slate-700 dark:text-slate-200">
        {value}
      </span>
...
function InfoRow({ icon: Icon, label, value, copyable, onCopy, copied }: any) {
  return (
    <div className="flex items-center gap-2 py-1.5 px-2.5 rounded-lg bg-slate-50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800/50">
      ...
      <span className="text-[10px] font-bold uppercase text-slate-400 tracking-wider min-w-[44px] shrink-0">
        {label}
      </span>
      <span className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate flex-1 text-right">
        {value}
      </span>
      {copyable && (
        <button
...
function AccountBadge({
  ...
  return (
    <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-50 dark:bg-slate-900/30 border border-slate-100 dark:border-slate-800/50">
      ...
      <GlowingBadge variant={variant || 'neutral'} className="text-[9px]">
        {value}
      </GlowingBadge>
    </div>
  )
