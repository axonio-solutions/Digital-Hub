...
import { Store, Package, TrendingUp, ArrowUpRight, ShieldCheck, Activity, Map } from "lucide-react"
    ...
    if (isLoading) {
        return (
            <div className="h-[60vh] flex flex-col items-center justify-center gap-4 animate-pulse">
                ...
                <p className="text-sm font-black text-slate-400 uppercase tracking-widest">Scanning Merchant Network...</p>
            </div>
        )
    }
    ...
    const metrics = [
        {
            ...
            value: analytics?.metrics?.activeSellers || 0,
        ...
        },
        {
            ...
            value: analytics?.metrics?.totalQuotes || 0,
        ...
        <div className="flex flex-col space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div className="space-y-1">
                    <h2 className="text-3xl font-black tracking-tight text-slate-900 uppercase italic">Seller Ecosystem</h2>
                    <p className="text-muted-foreground font-medium">Supply-chain logistics and merchant footprint analysis.</p>
                </div>
                <div className="flex gap-3">
                    <div className="px-5 py-2.5 bg-emerald-600 rounded-2xl text-white font-black text-xs uppercase tracking-widest shadow-lg shadow-emerald-200 flex items-center gap-2">
                        <Activity className="size-3" /> Real-time Logistics
                    </div>
                </div>
            </div>
                    ...
                    >
                        <div className="relative z-10 flex flex-col gap-4">
                            <div className={cn("size-12 rounded-xl flex items-center justify-center",
                                m.color === 'emerald' ? "bg-emerald-50 text-emerald-600 border border-emerald-100" :
                ...
                <Card className="lg:col-span-2 border-slate-200 shadow-sm rounded-3xl bg-white overflow-hidden p-8 flex flex-col gap-6 relative">
                    <div>
                        <CardTitle className="text-2xl font-black tracking-tight mb-2 flex items-center gap-2 text-slate-900">
                            <Map className="size-6 text-emerald-600" /> Key Trade Hubs
                        </CardTitle>
                        <p className="text-muted-foreground text-sm font-medium leading-relaxed">Top performing wilayas by confirmed merchant activity levels.</p>
                    </div>
                        ...
                        {analytics?.distribution?.sort((a: any, b: any) => b.count - a.count).slice(0, 6).map((item: any, idx: number) => (
                            <div key={item.wilaya} className="flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-100 hover:bg-slate-100/50 transition-colors">
                                <div className="flex items-center gap-4">
                                    <span className="size-8 rounded-lg bg-white border border-slate-200 text-emerald-600 flex items-center justify-center font-black text-xs shadow-sm">{idx + 1}</span>
                    ...
                    <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-100 text-emerald-700 text-[10px] font-bold leading-normal uppercase tracking-wider">
                        Logistic nodes are validated against commercial registry geolocation. Database updated in real-time.
                    </div>
                </Card>
            </div>
        </div>
