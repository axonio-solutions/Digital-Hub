...
function RequestsVsQuotesChart({
  data,
  dir,
}: {
  data: Array<{ date: string; requests: number; quotes: number }>
  dir: 'ltr' | 'rtl'
}) {
      ...
      <div className="h-[220px] sm:h-[300px] w-full">
        {isEmpty ? (
          ...
          <ChartContainer
            config={{
              requests: {
                label: t('charts.market_activity.requests'),
                color: '#3b82f6',
              },
              quotes: {
                label: t('charts.market_activity.quotes'),
                color: '#10b981',
              },
            }}
...
function CategorySupplyGapsChart({
  data,
  dir,
}: {
  data: Array<{
  ...
  const maxVal = useMemo(() => {
    const m = Math.max(
      ...displayData.map((d) => Math.max(d.demand, d.supplySellers)),
      1,
    )
      ...
      <div className="h-[220px] sm:h-[300px] w-full">
        {isEmpty ? (
          ...
          <ChartContainer
            config={{
              demand: {
                label: t('charts.supply_gaps.demand'),
                color: '#3b82f6',
              },
              supplySellers: {
                label: t('charts.supply_gaps.supply'),
                color: '#10b981',
              },
              ...
              >
                ...
                <YAxis
                  ...
                  tickFormatter={(val) => {
                    ...
                    return translated.length > maxLen
                      ? translated.slice(0, maxLen) + '…'
                      : translated
                  }}
                />
                <ChartTooltip
                ...
                <Bar dataKey="demand" radius={[0, 3, 3, 0]} maxBarSize={16}>
                  ...
                  <LabelList
                    dataKey="gap"
                    position="right"
                    className="text-[9px] sm:text-[10px] font-bold fill-muted-foreground"
                  />
                </Bar>
                <Bar
                  dataKey="supplySellers"
                  fill="#10b981"
                  radius={[0, 3, 3, 0]}
                  maxBarSize={16}
                />
...
export function AdminOverview() {
  ...
  const { data: stats, isLoading: isStatsLoading } =
    useAdminDashboardStats(days)
  const { data: systemMetrics, isLoading: isMetricsLoading } =
    useSystemMetrics(days)
  ...
  const isHealthy = (stats?.marketHealth ?? '')
    .toLowerCase()
    .includes('healthy')
  ...
  const gapCategoriesCount = (gapAnalysis?.categoryGaps || []).filter(
    (c: any) => c.gap > 0,
  ).length
    ...
    <div className="flex-1 flex flex-col gap-4 sm:gap-6 w-full pb-8 pt-2">
      <DirectionProvider dir={dir}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground font-medium">
                  {isHealthy
                    ? t('users.overview.thriving')
                    : t('users.overview.needs_attention')}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
          ...
          <StatCard
            icon={ClipboardList}
            label={t('stats.demands.label')}
            value={openRequests}
            color="text-amber-600 bg-amber-50 dark:bg-amber-950/30"
          />
          <StatCard
            icon={MessageSquare}
            label={t('stats.supply.label')}
            value={totalQuotes}
            color="text-violet-600 bg-violet-50 dark:bg-violet-950/30"
          />
          <StatCard
            icon={TrendingUp}
            label={t('users.overview.avg_offers')}
            value={avgOffers.toFixed(1)}
          ...
          <StatCard
            icon={UserPlus}
            label={t('users.overview.recent_registrations')}
            value={registrations}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
            ...
            <StatCard
              icon={HelpCircle}
              label={t('intelligence_page.metrics.unserved')}
              value={unservedCount.toLocaleString()}
              color="text-rose-600 bg-rose-50 dark:bg-rose-950/30"
            />
            <StatCard
              icon={ClipboardList}
              label={t('intelligence_page.category_gaps')}
              value={gapCategoriesCount}
              color="text-amber-600 bg-amber-50 dark:bg-amber-950/30"
            />
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:gap-4 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5 rounded-2xl bg-card border border-border shadow-sm p-3 sm:p-5 min-w-0">
            ...
            <CategorySupplyGapsChart
              data={gapAnalysis?.categoryGaps || []}
              dir={dir}
            />
          </div>
        </div>
