  ...
  return (
    <>
      <DropdownMenu>
        ...
        >
          <DropdownMenuLabel className="text-[10px] font-black uppercase tracking-widest text-muted-foreground py-2 rtl:text-end">
            {t('actions.title')}
          </DropdownMenuLabel>
          <DropdownMenuSeparator />
          <DropdownMenuItem
            onClick={() => onAction?.({ type: 'view_request', item: request })}
            className="gap-2.5 text-xs font-bold p-3 cursor-pointer rounded-xl rtl:flex-row-reverse"
          >
          ...
          {request.status === 'open' && (
            <DropdownMenuItem
              onClick={() =>
                onAction?.({ type: 'edit_request', item: request })
              }
              className="gap-2.5 text-xs font-bold p-3 cursor-pointer rounded-xl rtl:flex-row-reverse"
            >
              <SquarePen className="size-4 shrink-0" />{' '}
              {t('actions.edit_request')}
            </DropdownMenuItem>
          )}
  ...
  return useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: { row: any }) => (
          <div className="flex flex-col">
            <span
              className="font-medium text-sm truncate max-w-[180px] block"
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col min-w-0">
              ...
              <span className="text-xs text-muted-foreground">
                ...
                {row.original.brand?.clusterRegion || t('empty.general')}
              </span>
            </div>
          </div>
        ),
      ...
      },
      {
        ...
        cell: ({ row }: { row: any }) => {
          const count =
            row.original.quotesCount ?? row.original.quotes?.length ?? 0
          return (
            <div className="flex items-center gap-1.5">
              <MessageSquare
                className={`size-3 ${count > 0 ? 'text-primary' : 'text-muted-foreground'}`}
              />
              <span
                className={`text-sm font-bold ${count > 0 ? 'text-foreground' : 'text-muted-foreground'}`}
              >
                {count}
              </span>
            </div>
      ...
      },
      {
        ...
        cell: ({ row }: { row: any }) => (
          <div
            className="sticky right-0 pl-2 -mr-2"
