...
import {
  useSellerSpecialties,
  useUpdateSellerSpecialties,
...
export function SpecialtyManager() {
  ...
  const { data: currentSpecialties, isLoading: loadingCurrent } =
    useSellerSpecialties()
  ...
  const isDirty = useMemo(() => {
    ...
    const categoriesMatch = selectedCategories.every((id) =>
      initialCategories.includes(id),
    )
    ...
    <Card className="border shadow-sm bg-card">
      <CardHeader className="border-b">
        <CardTitle className="text-xl font-bold tracking-tight">
          {t('specialties.title', 'Marketplace Specialties')}
        </CardTitle>
        <CardDescription>
          {t(
            'specialties.desc',
            'Select the car brands and parts categories you specialize in.',
          )}
        </CardDescription>
      </CardHeader>
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="rounded-full">
              {selectedBrands.length} {t('specialties.selected', 'Selected')}
            </Badge>
          </div>
                ...
                >
                  {brand.imageUrl ? (
                    <img
                      ...
                      alt={brand.brand}
                      className={cn(
                        'size-5 object-contain shrink-0',
                  ...
                  {isSelected ? (
                    ...
                    <Plus className="size-3 opacity-40 shrink-0" />
                  )}
                </button>
              )
            })}
        ...
        <section className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Badge variant="outline" className="rounded-full">
              ...
              {t('specialties.selected', 'Selected')}
            </Badge>
          </div>
          <div className="flex flex-wrap gap-2">
