...
export function DataTableToolbar<TData>({
  ...
  const availableFilters = facetedFilters.filter((f) => table.getColumn(f.column))
  ...
  return (
    <div className={cn("flex flex-col gap-2 md:flex-row md:items-center md:gap-3", className)}>
      ...
      <div className="flex items-center gap-2 w-full md:w-auto overflow-hidden">
        ...
        <div className="flex items-center gap-1.5 md:hidden shrink-0">
          {availableFilters.length > 0 && (
            <Drawer shouldScaleBackground={false}>
              <DrawerTrigger asChild>
                <Button variant="outline" size="sm" className="h-10 gap-1.5 rounded-xl border-border/60 bg-card text-sm font-medium">
              ...
              <DrawerContent className="flex flex-col" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
                ...
                {isFiltered && (
                  <div className="px-4 pt-2 pb-3">
                    <Button variant="secondary" size="sm" onClick={() => table.resetColumnFilters()} className="h-10 w-full rounded-xl gap-2">
                      <X className="size-4" />
                      {t('table.reset')}
                    </Button>
                  </div>
          ...
          {!hideViewOptions && (
            <Drawer shouldScaleBackground={false}>
              <DrawerTrigger asChild>
                <Button variant="outline" size="icon" className="h-10 w-10 rounded-xl border-border/60 bg-card shrink-0">
                  <Columns2 className="size-4" />
                </Button>
              </DrawerTrigger>
              <DrawerContent className="flex flex-col" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
                <DrawerHeader className="text-left pb-2">
                  <DrawerTitle>{t('table.toggle_columns', 'Columns')}</DrawerTitle>
                </DrawerHeader>
      ...
      <div className="hidden md:flex md:items-center md:gap-2 overflow-hidden">
        ...
        {isFiltered && (
          <Button variant="ghost" size="sm" onClick={() => table.resetColumnFilters()} className="h-9 rounded-xl text-muted-foreground hover:text-foreground hover:bg-accent gap-1.5 animate-in fade-in">
