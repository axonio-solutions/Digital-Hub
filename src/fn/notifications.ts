...
export const getNotificationPreferencesServerFn = createServerFn({
  ...
  .handler(async ({ context }: { context: any }) => {
    const { getNotificationSettingsUseCase } =
      await import('@/use-cases/notifications/index')
...
export const fetchUnreadNotificationsServerFn = createServerFn({
  method: 'GET',
})
  .middleware([authMiddleware])
  .inputValidator((limit: number) =>
    typeof limit === 'number' && limit > 0 ? limit : 10,
  )
  .handler(async ({ data: limit, context }) => {
    const userId = context?.user?.id
    const { getUnreadNotificationsUseCase } =
      await import('@/use-cases/notifications/index')
    return await getUnreadNotificationsUseCase(userId, limit ?? 10)
  })
...
export const markNotificationReadServerFn = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data: id, context }) => {
    const { markNotificationAsReadUseCase } =
      await import('@/use-cases/notifications/index')
    return await markNotificationAsReadUseCase(id, context.user.id)
  })
