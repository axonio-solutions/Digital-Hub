...
import { MessageSquare, Award, MessageCircle, Phone, ArrowLeftCircle } from "lucide-react"
...
function OffersReceivedRoute() {
  ...
  if (isLoading) return <div className="flex h-64 items-center justify-center">Loading Offers...</div>
      ...
      <div className="grid gap-4 md:grid-cols-1">
        <Card className="col-span-1 shadow-sm border-primary/20 bg-primary/5">
          <CardHeader className="pb-3 border-b border-primary/10">
            ...
            <CardDescription>
              These requests require your attention. Accept a quote to finalize the deal.
            </CardDescription>
          </CardHeader>
                      ...
                      <TableCell colSpan={5} className="text-center py-12 text-muted-foreground">
                        <div className="flex flex-col items-center justify-center space-y-3">
                          ...
                          <Button variant="link" asChild>
                             <a href="/dashboard"><ArrowLeftCircle className="mr-2 h-4 w-4" /> Go back to dashboard</a>
                          </Button>
                        </div>
                      </TableCell>
                   </TableRow>
                  ...
                  <TableRow key={req.id}>
                    ...
                    <TableCell>
                      <Badge variant="default" className="bg-primary hover:bg-primary/80">
                         {req.quotes.length} Offer{req.quotes.length > 1 ? 's' : ''}
                      </Badge>
                    </TableCell>
                            ...
                            <div className="space-y-4 py-4">
                              <div className="grid gap-4">
                                {selectedRequest.quotes.map((quote: any) => (
                                  <div key={quote.id} className="border border-primary/20 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-background">
                            ...
                            <div className="py-6 space-y-6 text-center animate-in fade-in zoom-in duration-300">
                               ...
                               <div className="bg-muted/30 border rounded-xl p-6 max-w-md mx-auto space-y-4">
                                  <p className="text-sm text-balance text-muted-foreground mb-4">
                                    Contact the seller directly to arrange payment and logistics.
                                  </p>
                               ...
                               <Button variant="ghost" className="mt-4" onClick={() => setSelectedRequest(null)}>
                                 Close
                               </Button>
                            </div>
                          )}
