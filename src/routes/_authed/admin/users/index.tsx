...
function AdminUsersRoute() {
  ...
  const { totalUsers, pendingVerifications, activeSellers, waitlistedSellers } =
    useMemo(
      () => ({
        totalUsers: users.length,
        pendingVerifications: users.filter((u: any) => !u.emailVerified).length,
        activeSellers: users.filter(
          (u: any) => u.role === 'seller' && u.account_status === 'active',
        ).length,
        waitlistedSellers: users.filter(
          (u: any) => u.account_status === 'waitlisted',
        ).length,
      }),
      [users],
    )

  const handleBanToggle = useMemo(
    () => (userId: string, currentBanStatus: boolean) => {
  ...
  const metrics = useMemo(
    () => [
      {
        label: t('stats.registered.label'),
        value: totalUsers,
        icon: Users,
        color: 'text-blue-500 bg-blue-50 dark:bg-blue-950/30',
      },
      {
        label: t('stats.waitlisted.label'),
        value: waitlistedSellers,
        icon: Shield,
        color: 'text-amber-500 bg-amber-50 dark:bg-amber-950/30',
      },
      {
        label: t('stats.active_sellers.label'),
        value: activeSellers,
        icon: Store,
        color: 'text-emerald-500 bg-emerald-50 dark:bg-emerald-950/30',
      },
      {
        label: t('stats.pending_email.label'),
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 w-full pb-8 pt-2">
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Button
            disabled
            variant="outline"
            className="h-10 px-4 rounded-xl border-slate-200 dark:border-slate-800 gap-2 font-bold text-xs uppercase tracking-wider shadow-sm opacity-50 cursor-not-allowed"
          >
            <Download size={14} /> {t('buttons.export')}
          </Button>
          <Button
            disabled
            className="h-10 px-4 rounded-xl bg-primary text-primary-foreground gap-2 font-bold text-xs uppercase tracking-wider shadow-sm opacity-50 cursor-not-allowed"
          >
      ...
      <div className="space-y-3">
        ...
        <AdminUsersTable
          users={users}
          onBan={(id: string) => handleBanToggle(id, false)}
        />
      </div>
    </div>
  )
